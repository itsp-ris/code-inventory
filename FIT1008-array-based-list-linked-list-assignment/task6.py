...
class Editor:
    def __init__(self):
        '''
        function build a LinkedList
        precondition: file with LinkedList class is imported
        :param:
        postcondition:
        :return:
    ...
    def search_word(self, word):
        ...
        complexity: best: O(N**2) where N is the length of list
                    worst: O(N**3) where N is the length of list
        '''
        assert len(self.array) != 0, 'Please read in a file first.'
        num = ''
        punctuation = '''`~!@#$%^&*()_+-={}[]|\:;<>?,./'''
        ...
        # when word is not in the list, both first loop and contains function runs n times
        ...
        for j in range(len(self.array)):
            ...
            if search_word in self.array[j].lower():
                for k in range(len(self.array[j])):
                    if self.array[j][k] not in punctuation:
