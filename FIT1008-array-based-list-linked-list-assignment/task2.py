...
class Array:
    ...
    def resize(self):
        '''
        function resizes array
        precondition: list is full or list is filled less than 1/8 of its size
        :param:
        postcondition: items in the list remains after resizing
        :return: address of caller
        complexity: best and worst: O(N) where N is the length of list
        '''
        if self.is_full():
            self.size *= 2
            other = build_array(self.size)
            for i in range(len(self)):
                other[i] = self.array[i]
            self.array = other

        elif self.size/2 >= 20 and self.count/self.size < 1/8:
            self.size //= 2
            other = build_array(self.size)
            for i in range(len(self)):
                other[i] = self.array[i]
    ...
    def delete(self, index):
        ...
        if not self.is_empty():
            if index >= 0:
                ...
                for i in range(index + len(self), len(self)):
                    self.array[i] = self.array[i + 1]
            self.count -= 1
