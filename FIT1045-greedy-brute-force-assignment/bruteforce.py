...
argument: startVertices: the list of possible start vertices.

returns: this function does not return anything. '''
def makePath(start, graph, path, pathList, checkpoint, endVertices, startVertices):
    path += [start]

    if start in endVertices:
        newpath = path[:]
        appending(pathList, newpath)
        return pathList
        ...
        for vertex in graph[start]:
            ...
            if vertex not in startVertices:

                if vertex not in path:
                    pathway = makePath(vertex, graph, path, pathList, start, endVertices, startVertices)
                    path.remove(path[-1])
...
if response != 'True':
    file4 = open(('bruteforce' + graph1 + intermediateGraph + graph2 + '.txt'), 'w')

    file4.close()

    file5 = open(('bruteforce' + graph1 + intermediateGraph + graph2 + 'connectivity.txt'), 'w')

    file5.close()

    file6 = open(('bruteforce' + graph1 + intermediateGraph + graph2 + 'cost.txt'), 'w')
    ...
    for i in range(vertex4.index(intermediateVertex[0]), vertex4.index(graph2Vertex[-1])):
        ...
        for j in range(vertex4.index(intermediateVertex[0]), vertex4.index(graph2Vertex[-1])):
            ...
            if adjMat4[i][j] > 0:
                graphConnection1.setdefault(vertex4[i], []).append(vertex4[j])
    ...
    for i in range(len(intermediateVertex)):
        ...
        if intermediateVertex[i] in graph1Vertex:
            makePath(intermediateVertex[i], graphConnection1, path, pathList, intermediateVertex[i], graph2Vertex, graph1Vertex)
    ...
    for i in range(len(graph1Vertex)):
        appending(newVertex, graph1Vertex[i])

    for i in range(len(intermediateVertex)):
        if intermediateVertex[i] in cheapestPath:
            appending(newVertex, intermediateVertex[i])

    for i in range(len(graph2Vertex)):
        appending(newVertex, graph2Vertex[i])
    ...
    file6 = open(('bruteforce' + graph1 + intermediateGraph + graph2 + 'pathcost.txt'), 'w')
