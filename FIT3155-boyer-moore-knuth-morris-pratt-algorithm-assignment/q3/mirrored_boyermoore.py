...
def computeShift(txt, right_leftmost, char, z_arr, good_prefix, matched_suffix, i, j, pat_len):
    ...
    if char[ord(txt[i + j]) - 32] > 0:
        if good_prefix[j] >= right_leftmost[char[ord(txt[i + j]) - 32] - 1][j] > 0:
            ...
            # previous[1] = z_arr[good_prefix[previous[1]]]
            ...
            # previous[1] = matched_suffix[previous[1]]
    else:
        if good_prefix[j] > 0:
            i, j = goodPrefixRule(good_prefix, i, j)
            # previous[1] = z_arr[good_prefix[previous[1]]]
...
def mirrored_boyermoore(txtfile, patfile):
    ...
    # print(output[::-1])
    out = [n for n in range(len(text)) if text.find(pat, n) == n]
    # print(out)
    print(len(output), len(out), len(pat), pat)
...
if __name__ == '__main__':
    # main()
    text = openFile('reference.txt')
    patterns = openFile('pattern.txt').split()
    for i in range(len(patterns)):
        mirrored_boyermoore(text, patterns[i])
    # mirrored_boyermoore(text, patterns[1])
