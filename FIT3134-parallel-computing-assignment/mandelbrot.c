...
int main()
 {
	 /* Clock information */
	clock_t start, end, oEnd, oStart = clock();
	double cpu_time_used, o_time_used;

	/* screen ( integer) coordinate */
	int iX,iY;
	const int iXmax = 4000; // default
	const int iYmax = 4000; // default
	...
	int i = 0;
	unsigned char *writeArray = (char*) malloc(iYmax*iXmax*3*sizeof(unsigned char));
	...
	fwrite(writeArray, iXmax*iYmax*3*sizeof(unsigned char), 1, fp);

	fclose(fp);

	oEnd = clock();
	o_time_used = ((double)(oEnd - oStart)) / CLOCKS_PER_SEC;
	printf("Completed Computing Mandelbrot Set.\n");
	printf("File: %s successfully closed.\n", filename);
	printf("Mandelbrot computational process time: %lf\n", cpu_time_used);
	printf("Mandelbrot overall process time: %lf\n", o_time_used);
