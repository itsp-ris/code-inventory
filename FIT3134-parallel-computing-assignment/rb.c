...
int writeFile(unsigned char* writeArray, int *sz, int *rem, int *eppr, const int *iYmax, const int *iXMax, int *size, int *rank);
...
{	
    ...
    writeFile(writeArray, &sz, &rem, &eppr, &iYmax, &iXmax, &size, &rank);

    if (rank == 0) {
        fwrite(writeArray, sz*sizeof(unsigned char), 1, fp);
        for (p = 1; p < size; p++) {
		...
		if (rem > 0) {
			...
			fwrite(writeArray, sizeof(unsigned char), rem*iXmax*3, fp);
		}
